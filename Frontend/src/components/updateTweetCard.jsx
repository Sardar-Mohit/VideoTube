  ...
  const submitForm = async (event) => {
    ...
    try {
      if (formData.content === content) {
        setError("You should change content to update playlist.");
        return false;
      }
      ...
      const request = await updateTweetApi(tweetId, formData);
      console.log(request);
      getUserTweets();
      close();
    } catch (error) {
      if (error.response === 500) {
        setError("Something went wrong while updating tweet details.");
      }
      throw error;
    } finally {
      setLoading(false);
...
export default UpdateTweetCard;
